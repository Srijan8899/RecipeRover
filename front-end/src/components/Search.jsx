...
function Search({currentpage}) {
  ...
  const placeholderText = currentpage === "home" ? "Search by Ingredient" : "Search by Recipe Name"
  return (
    ...
    <div className="relative inset-0 flex items-center justify-between px-4 py-1 leading-none bg-[#F8FAE5] rounded-xl overflow-hidden">
        <input onChange={e => setSearch(e.target.value)} value={search} id="search" className="bg-[#F8FAE5] text-xl w-[35vw] pl-2 focus:ring-[#F8FAE5] border-none focus:outline-none" type="text" name="search" placeholder={placeholderText}  />
            <button type="submit" onClick={submitHandler} className=" input-group-text flex items-center whitespace-nowrap rounded px-3 py-1.5 text-center text-base font-normal text-neutral-700"><FaSearch size={35}/></button>
        </div>
        </div>
    </div>
    </div>
